    ...
    case rpmostreecxx::RefspecType::Container:
      {
        if (check)
          {
            CXX_TRY_VAR (changed,
                         rpmostreecxx::check_container_update (*self->repo, *cancellable,
                                                               r.refspec.c_str ()),
                         error);
            *out_changed = changed;
            return TRUE;
 ...
 *
 * The source ownership is stored as a special key in the BLS config:
                                   ...
                                   GError **error)
{
   ...
   * a "bootconfig-extra" key so the custom BLS keys survive finalization
   * at shutdown. For direct deployments, we write the BLS file directly.
   */
  if (self->kargs_source != NULL)
    {
      g_autofree char *source_key = g_strdup_printf ("ostree-source-%s", self->kargs_source);
      ...
      if (use_staging)
        {
          /* Update the staged deployment GVariant with the extra bootconfig key.
           * During finalization at shutdown, ostree reads "bootconfig-extra" from
           * the staged data and restores these keys on the deployment's bootconfig
           * before writing the final BLS entry.
           */
          static const char staged_path[] = "/run/ostree/staged-deployment";

          g_autoptr (GMappedFile) mfile
              = g_mapped_file_new (staged_path, FALSE, error);
          if (!mfile)
            return glnx_prefix_error (error, "Reading staged deployment data");

          g_autoptr (GBytes) contents
              = g_mapped_file_get_bytes (mfile);
          ...
          /* Build the bootconfig-extra a{ss} variant with our source key */
          g_autoptr (GVariantBuilder) extra_builder
              = g_variant_builder_new ((GVariantType *)"a{ss}");

          /* Preserve any existing extra keys from the staged data */
          g_autoptr (GVariant) existing_extra = NULL;
          if (g_variant_dict_lookup (dict, "bootconfig-extra", "@a{ss}", &existing_extra))
            {
              GVariantIter iter;
              const char *k, *v;
              g_variant_iter_init (&iter, existing_extra);
              while (g_variant_iter_next (&iter, "{&s&s}", &k, &v))
                {
                  /* Skip our own source key — we'll add the updated value below */
                  if (!g_str_equal (k, source_key))
                    g_variant_builder_add (extra_builder, "{ss}", k, v);
                }
            }

          /* Add our source key */
          g_variant_builder_add (extra_builder, "{ss}", source_key, source_val);

          g_variant_dict_insert_value (dict, "bootconfig-extra",
      ...
      else
        {
          ...
          if (new_bootconfig != NULL)
            {
              ostree_bootconfig_parser_set (new_bootconfig, source_key, source_val);

                  ...
                  GError         **error)
{
  ...
  /* Convert metadata hashes to GVariants */
  ...
  if (g_hash_table_size(self->detached_metadata) > 0)
    {
      g_autoptr(GVariantBuilder) detached_metadata_builder = g_variant_builder_new (G_VARIANT_TYPE ("a{sv}"));

      GLNX_HASH_TABLE_FOREACH_KV (self->detached_metadata, const char*, strkey, GVariant*, v)
        g_variant_builder_add (detached_metadata_builder, "{sv}", strkey, v);

      detached_metadata = g_variant_ref_sink (g_variant_builder_end (detached_metadata_builder));
      rpmostree_variant_native_to_be (&detached_metadata);
    }
  ...
  if (!rpmostree_compose_commit (self->rootfs_dfd, self->build_repo, parent_revision,
                                 metadata, detached_metadata, gpgkey, selinux, self->devino_cache,

...
/* Like realpath(3), but ensures we stay within the given rootfs. We assume the
 * rootfs is already made canonical. */
                                  ...
                                  GError       **error)
{
  ...
  g_return_val_if_fail (rootfs != NULL, NULL);
  g_return_val_if_fail (relpath != NULL, NULL);
  g_return_val_if_fail (relpath[0] != '/', NULL);

  path = g_build_filename (rootfs, relpath, NULL);

  if (TEMP_FAILURE_RETRY (lstat (path, &stbuf)) != 0)
    {
      glnx_set_error_from_errno (error);
      return NULL;
    }
  ...
  if (S_ISREG (stbuf.st_mode) || \
    ...
    {
      g_autofree char *real = realpath (path, NULL);
      if (g_str_has_prefix (real, rootfs)) /* NB: rootfs is canonical */
        return g_steal_pointer (&real);

      /* NB: Consider changing this to e.g. NOT_FOUND if callers need to
       * special-case this. */
      g_set_error_literal (error, G_IO_ERROR, G_IO_ERROR_FAILED,
                           "File falls outside rootfs");
      return NULL;
    }
  else if (S_ISLNK (stbuf.st_mode))
    {
      g_autofree char *link = NULL;
      g_autofree char *linkrel = NULL;

      link = glnx_readlinkat_malloc (AT_FDCWD, path, cancellable, error);
      if (link == NULL)
        {
          g_prefix_error (error, "While resolving %s: ", path);
          return NULL;
        }

      if (link[0] == '/')
        linkrel = g_strdup (g_path_skip_root (link));
      else
        {
          g_autofree char *dup = g_strdup (relpath);
          const char *dir = dirname (dup);
          linkrel = g_build_filename (dir, link, NULL);
        }

      return rpmostree_realpath_within_rootfs (rootfs, linkrel,
                                               cancellable, error);
    }
  else
    {

...
fn parse_booted_oci() -> Result<()> {
    ...
    let base_commit_meta = String::deserialize(booted.base_commit_meta)

    println!("{}", booted.base_commit_meta.len());
    assert!(booted.find_base_commitmeta_string("fedora-coreos.stream").is_ok());
    assert_eq!(
        booted
            .find_base_commitmeta_string("coreos-assembler.config-gitrev")
            .unwrap(),

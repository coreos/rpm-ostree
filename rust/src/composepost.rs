...
#[context("Postprocessing basic sysusers.d configuration")]
fn postprocess_basic_sysusers_conf(rootfs_dfd: &cap_std::fs::Dir) -> Result<()> {
    static SYSUSERS_D_PATH: &str = "usr/lib/sysusers.d";
    static CONF_FILENAME: &str = "00-rpm-ostree_basic.conf";

    {
        let mut db = cap_std::fs::DirBuilder::new();
        db.recursive(true);
        db.mode(0o644);
        rootfs_dfd.create_dir_with(SYSUSERS_D_PATH, &db)?;
    }

    let sysusers_dfd = rootfs_dfd.open_dir(SYSUSERS_D_PATH)?;
    sysusers_dfd.atomic_replace_with(&CONF_FILENAME, |w| -> Result<()> {
        let perms = cap_std::fs::Permissions::from_mode(0o644);
        w.get_mut().as_file_mut().set_permissions(perms)?;

        let static_ids = crate::StaticEntries::new();
        for line in static_ids.sysusers_entries() {
...
pub fn compose_postprocess_final(rootfs_dfd: i32) -> CxxResult<()> {
    ...
    let tasks = [
        postprocess_basic_sysusers_conf,
        postprocess_useradd,
        postprocess_presets,

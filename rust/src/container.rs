...
const COMPONENT_XATTR: &[u8] = b"user.component\0";
...
impl MappingBuilder {
    /// Create an ObjectMeta for package-based content (excluding exclusive components)
    fn create_package_meta(&self) -> ObjectMeta {
        let mut content = ObjectMetaMap::default();
        for (checksum, paths) in &self.checksum_paths {
            // Find content ID for this checksum, excluding components
            let content_id = paths
                .iter()
                .filter_map(|p| self.path_packages.get(p))
                .flatten()
                .find(|id| {
                    // Exclude component IDs - only use package IDs
                    !self
                        .componentmeta
                        .iter()
                        .any(|meta| &meta.identifier == *id)
                })
                .unwrap_or(&self.unpackaged_id);

            content.insert(checksum.clone(), content_id.clone());
        }
        ...
        ObjectMeta {
            ...
            set: self
                .packagemeta
                .iter()
                .map(|meta| ObjectSourceMeta {
                    identifier: meta.identifier.clone(),
                    name: meta.name.clone(),
                    srcid: meta.srcid.clone(),
                    change_time_offset: meta.change_time_offset,
    ...
    /// Create an ObjectMeta for exclusive component content only
    fn create_component_meta(&self) -> ObjectMeta {
        let mut content = ObjectMetaMap::default();
        for (checksum, paths) in &self.checksum_paths {
            // Find content ID for this checksum, only components
            if let Some(content_id) = paths
                .iter()
                .filter_map(|p| self.path_packages.get(p))
                .flatten()
                .find(|id| {
                    // Only include component IDs
                    self.componentmeta
                        .iter()
                        .any(|meta| &meta.identifier == *id)
                })
...
pub fn container_encapsulate(args: Vec<String>) -> CxxResult<()> {
    ...
    let package_meta = state.create_package_meta();
    let component_meta = state.create_component_meta();
...
mod tests {
    ...
    fn test_mapping_builder_create_package_meta() {
        ...
        builder.path_packages.insert(path2.clone(), {
            let mut set = BTreeSet::new();
        ...
        let package_meta = builder.create_package_meta();

        // Package meta should contain both checksums but component content should be unpackaged
        assert_eq!(package_meta.map.len(), 2);
        assert!(package_meta.map.contains_key(&checksum1));
        assert!(package_meta.map.contains_key(&checksum2));
        assert_eq!(package_meta.map[&checksum1], pkg_id);
        assert_eq!(package_meta.map[&checksum2], builder.unpackaged_id); // Component content becomes unpackaged

        // Should have package metadata but not component metadata
    ...
    fn test_mapping_builder_create_component_meta() {
        ...
        let component_meta = builder.create_component_meta();

        // Component meta should only contain component content, not package content
        assert_eq!(component_meta.map.len(), 1);
        assert!(component_meta.map.contains_key(&checksum2));
        assert_eq!(component_meta.map[&checksum2], comp_id);
    ...
    fn test_mapping_builder_mixed_content() {
        ...
        // Component meta should be empty since no component content exists
        assert_eq!(component_meta.map.len(), 0);
    }
    ...
    fn test_get_user_component_xattr() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let root_repo_file = root
            .downcast::<ostree::RepoFile>()
            .map_err(|_| "Failed to downcast root to RepoFile")?;
        ...
        let repo_file = test_file
            .downcast::<ostree::RepoFile>()
            .map_err(|_| "Failed to downcast test-file to RepoFile")?;
        ...
        let root_repo_file = commit_root
            .downcast::<ostree::RepoFile>()
            .map_err(|_| "Failed to downcast root to RepoFile")?;

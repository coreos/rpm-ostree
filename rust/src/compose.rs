...
pub(crate) struct BuildChunkedOCIOpts {
    ...
    /// key.  The key is specified as <KEYTYPE>=/path/to/key or
...
mod tests {
    ...
    fn write_commit() -> Result<()> {
        ...
        let commit =
            generate_commit_from_rootfs(&repo, &td, modifier.clone(), None, false).unwrap();
        ...
        let commit =
            generate_commit_from_rootfs(&repo, &td, modifier.clone(), Some(&ts), false).unwrap();
        assert_eq!(
            commit,
